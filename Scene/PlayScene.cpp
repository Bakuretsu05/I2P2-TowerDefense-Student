...
void PlayScene::OnKeyDown(int keyCode) {
	...
	if (keyCode == ALLEGRO_KEY_Q) {
		...
		// Hotkey for SplashTurret
		UIBtnClicked(3);
	}
	// TODO: [CUSTOM-TURRET]: Make specific key to create the turret. DONE
...
void PlayScene::ConstructUI() {
	...
	// Button 4 (Splash Turret)
	btn = new TurretButton("play/floor.png", "play/dirt.png",
		Engine::Sprite("play/tower-base.png", 1294, 212, 0, 0, 0, 0),
		Engine::Sprite("play/SplashTurret.png", 1294, 212 - 8, 0, 0, 0, 0)
		, 1294, 212, SplashTurret::Price);
	btn->SetOnClickCallback(std::bind(&PlayScene::UIBtnClicked, this, 3));
	UIGroup->AddNewControlObject(btn);
	// TODO: [CUSTOM-TURRET]: Create a button to support constructing the turret. DONE
...
void PlayScene::UIBtnClicked(int id) {
	if (preview)
		UIGroup->RemoveObject(preview->GetObjectIterator());
    // TODO: [CUSTOM-TURRET]: On callback, create the turret. DONE
	...
	else if (id == 3 && money >= SplashTurret::Price)
...
std::vector<std::vector<int>> PlayScene::CalculateBFSDistance() {
	...
	while (!que.empty()) {
		...
		que.pop();
		// TODO: [BFS PathFinding] (1/1): Implement a BFS starting from the most right-bottom block in the map. DONE
		//               For each step you should assign the corresponding distance to the most right-bottom block.
		//               mapState[y][x] is TILE_DIRT if it is empty.
		int dx[] = {0, 0, -1, 1};
		int dy[] = {-1, 1, 0, 0};
		for(int i = 0; i < 4; i++) {
			if(p.x + dx[i] >= 0 && p.x + dx[i] < MapWidth && p.y + dy[i] >= 0 && p.y + dy[i] < MapHeight && mapState[p.y + dy[i]][p.x + dx[i]] == TILE_DIRT && map[p.y + dy[i]][p.x + dx[i]] == -1) {
				que.push(Engine::Point(p.x + dx[i], p.y + dy[i]));
